...
public class EntitySpawner
{
	...
	/**Uses entity's position to reset origin every tick*/
	...
	public void run()
	{	
		if (this.entityClass.getSuperclass() == MobEvents.proxy.getEntityFXClass() && this.world.isRemote)
			ParticleUpdateEvent.addOnClientUpdate(this);
		else if (this.entityClass.getSuperclass() != MobEvents.proxy.getEntityFXClass() && !this.world.isRemote)
			ParticleUpdateEvent.addOnServerUpdate(this);
		this.isRunning = true;
	...
	public void onUpdate()
	{
		if (isRunning)
		{
			...
			{
				Vec3 newOrigin = new Vec3(entityToFollow.posX, entityToFollow.posY, entityToFollow.posZ);
				if (!this.origin.equals(newOrigin))
			...
			if (totalDelay < 0)
			{
				isRunning = false;
				if (this.entityClass.getSuperclass() == MobEvents.proxy.getEntityFXClass() && this.world.isRemote)
					ParticleUpdateEvent.removeOnClientUpdate(this);
				else if (this.entityClass.getSuperclass() != MobEvents.proxy.getEntityFXClass() && !this.world.isRemote)
					ParticleUpdateEvent.removeOnServerUpdate(this);
			}
		}
	}
	...
	public void spawnEntity(int heightIterator, int entityIterator)
	{
		...
		if (this.entityClass.getSuperclass() == MobEvents.proxy.getEntityFXClass() && this.world.isRemote)
		{
			MobEvents.proxy.spawnEntitySpawner(this.entityClass, this.world, this.x, this.y, this.z, this, this.heightIterator, this.entityIterator);
		}
		else if (this.entityClass.getSuperclass() != MobEvents.proxy.getEntityFXClass() && !this.world.isRemote)
		{
			try 
