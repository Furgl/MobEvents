...
public class ChaoticTurmoil extends Event
{
	...
	public void onUpdate() {	
		//speed up time
		if (!world.isRemote && !Minecraft.getMinecraft().isGamePaused()) 
			world.setWorldTime(world.getWorldTime()+(world.isDaytime() ? 30 : 10));
	...
	public void startEvent() { 
		super.startEvent();
		if (!world.isRemote) {
			this.sendServerMessage(new TextComponentTranslation("You asked for it...").setStyle(new Style().setBold(true).setColor(this.enumColor).setItalic(true)));	
			this.playServerSound(SoundEvents.ENTITY_ZOMBIE_INFECT, 10f, 0f);
		}
	}
	...
	public void stopEvent()	{
		super.stopEvent();
		if (!world.isRemote) {
			this.sendServerMessage(new TextComponentTranslation(this.toString() + " has ended.").setStyle(new Style().setBold(true).setColor(this.enumColor)));
			this.playServerSound(SoundEvents.ENTITY_ZOMBIE_VILLAGER_CURE,  0.2f, 2f);
		}
	}
