...
public class LibrarianChatEvent 
{
	...
	{
		if (MobEvents.proxy.getWorldData().currentEvent == Event.ZOMBIE_APOCALYPSE && MobEvents.proxy.getWorldData().currentEvent.boss != null && event.getPlayer() != null)
		{
			for (EntityBossZombie boss : event.getPlayer().worldObj.getEntitiesWithinAABB(EntityBossZombie.class, MobEvents.proxy.getWorldData().currentEvent.boss.getEntityBoundingBox().expand(65, 65, 65)))
			{
				boss.setAttackTarget(event.getPlayer());
				if (boss.type == 4 && !event.getPlayer().worldObj.isRemote)
				{
					Event.sendServerMessage(new TextComponentTranslation(boss.getName()+": "+boss.librarianTaunts.get(boss.worldObj.rand.nextInt(boss.librarianTaunts.size()))).setStyle(new Style().setColor(boss.getChatColor()).setItalic(true)));
					for (int i=0; i<100; i++)
