...
public class EntityThiefZombie extends EntityEventZombie
{
	...
	public void setBookDescription()
	{
		...
		this.addDrops(Items.gold_ingot, 10);
		this.addDrops(Item.getItemFromBlock(Blocks.tnt), 10);
		this.addDrops(Items.emerald, 3);
		this.addDrops(Items.diamond, 3);
	...
	public void onDeath(DamageSource cause)
	{
		if (!this.worldObj.isRemote && this.playerStolenFrom != null) {
			playerStolenFrom.addChatMessage(new ChatComponentTranslation("Your "+this.getHeldItem().getDisplayName()+" has been returned.").setChatStyle(new ChatStyle().setItalic(true).setColor(EnumChatFormatting.DARK_GREEN)));
			if (!this.playerStolenFrom.inventory.addItemStackToInventory(this.getHeldItem()))
				this.playerStolenFrom.entityDropItem(this.getHeldItem(), 0);
		}
		super.onDeath(cause);
	}
	...
	public boolean attackEntityAsMob(Entity entityIn)
	{
		...
		if (this.getHeldItem() == null && entityIn instanceof EntityPlayer && !(entityIn instanceof FakePlayer))
		{
			...
			if (itemSlots.size() > 0) {
				...
				this.setCurrentItemOrArmor(0, player.inventory.removeStackFromSlot(itemSlots.get(random.nextInt(itemSlots.size()))));
				this.playerStolenFrom = player;
				this.targetTasks.addTask(1, new EntityAIAvoidEntity(this, EntityPlayer.class, 20.0F, 1.1D, 1.5D));
				this.targetTasks.removeTask(attackAI);
				attackAI = null;
				player.addChatMessage(new ChatComponentTranslation("A Zombie Thief has stolen your "+this.getHeldItem().getDisplayName()+"!").setChatStyle(new ChatStyle().setItalic(true).setColor(EnumChatFormatting.DARK_RED)));
			}
		}
		return rt;
	}
	...
	public void onUpdate() 
	{
		...
		if (this.getHeldItem() != null && this.attackAI != null) {
			this.targetTasks.addTask(1, new EntityAIAvoidEntity(this, EntityPlayer.class, 20.0F, 1.1D, 1.5D));
			this.targetTasks.removeTask(attackAI);
			attackAI = null;
		}
		//load playerStolenFrom
		if (!this.worldObj.isRemote && this.getHeldItem() != null && this.playerStolenFrom == null && this.tagToRead != null) {
			if (!this.tagToRead.getString(MobEvents.MODID+": playerStolenFrom").isEmpty()) 
				this.playerStolenFrom = MinecraftServer.getServer().getConfigurationManager().getPlayerByUUID(UUID.fromString(this.tagToRead.getString(MobEvents.MODID+": playerStolenFrom")));
			if (this.playerStolenFrom != null)
				this.tagToRead = null;
		}
		//no target when stealing
		if (!this.worldObj.isRemote && this.getHeldItem() != null && this.getAttackTarget() != null)
	...
	{ 
		this.setCurrentItemOrArmor(4, ((ItemThievesMask) ModItems.thievesMask).getItemStack());
		this.setEquipmentDropChance(0, 0f);
		this.setEquipmentDropChance(4, 0f);
